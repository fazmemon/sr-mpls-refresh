#!/usr/bin/env python3
"""TARGET architecture (for approval) v2 — bl1 as border-leaf (leaf->core),
dedicated L3 PIM-SSM market-data segment (no VXLAN), director peers BOTH ASes."""

VBW, VBH = 1470, 1500

# name: (cx, cy, role, [l1,l2,l3])
NODES = {
    "spine1": (360,285,"dc",  ["spine1","OSPF-SR · EVPN-RR","lo 10.0.0.1"]),
    "spine2": (720,285,"dc",  ["spine2","OSPF-SR · EVPN-RR","lo 10.0.0.2"]),
    "leaf1":  (285,470,"dc",  ["leaf1","VTEP · OSPF-SR","lo 10.0.0.3"]),
    "leaf2":  (560,470,"dc",  ["leaf2","VTEP · OSPF-SR","lo 10.0.0.4"]),
    "bl1":    (860,470,"sr1", ["bl1","border-leaf · OSPF-SR","lo 10.0.0.6"]),
    "h1":     (285,620,"host",["h1","tenant","172.16.0.10"]),
    "h2":     (560,620,"host",["h2","tenant","172.16.0.11"]),
    "mdrx":   (1090,470,"mcast",["md-rx","PIM-SSM rx · no VXLAN","10.9.0.0/24"]),
    "core1":  (460,845,"sr1", ["core1","OSPF-SR core · ASBR1","lo 10.0.0.7"]),
    "core2":  (775,845,"sr1", ["core2","OSPF-SR · TI-LFA/FlexAlgo","lo 10.0.0.8"]),
    "asbr2":  (460,1120,"sr2",["asbr2","ASBR · OSPF-SR","lo 10.2.0.1"]),
    "edge2":  (720,1215,"sr2",["edge2","SR egress · EPE SIDs","lo 10.2.0.2"]),
    "peer1":  (575,1410,"peer",["peer1  (P)","peer · AS 65100","FRR"]),
    "transit1":(870,1410,"tran",["transit1  (T)","transit · AS 65200","FRR"]),
    "mdsrc":  (1035,1215,"mcast",["mktdata","PIM-SSM source","exchange feed"]),
    "td":     (1300,860,"ctrl", ["Traffic Dictator","SR-TE controller","BGP-LS + SR-Policy"]),
}

# (a, b, kind, label, tlabel)
LINKS = [
    ("leaf1","spine1","dc","",.5),("leaf1","spine2","dc","",.5),
    ("leaf2","spine1","dc","",.5),("leaf2","spine2","dc","",.5),
    ("bl1","spine1","dc","",.5),("bl1","spine2","dc","",.5),
    ("h1","leaf1","host","",.5),("h2","leaf2","host","",.5),
    ("bl1","mdrx","mcast","",.5),
    ("bl1","core1","sr1","",.5),("bl1","core2","sr1","",.5),
    ("core1","core2","sr1","",.5),
    ("core1","asbr2","interas","BGP-LU · inter-AS SR",.30),
    ("asbr2","edge2","sr2","",.5),
    ("edge2","peer1","epe","eBGP · EPE",.5),
    ("edge2","transit1","epet","eBGP · EPE",.5),
    ("edge2","mdsrc","mcast","PIM-SSM feed",.5),
    ("td","core1","ctrl","BGP-LS / SR-Policy",.30),
    ("td","asbr2","ctrl","… into AS 65002",.62),
]
...
ROLE = {
    ...
    "mcast":dict(w=156,h=62,fill="var(--mcast-fill)",stroke="var(--mcast)"),
...
out=[]; A=out.append; link_svg=[]; label_svg=[]
for a,b,kind,lab,tl in LINKS:
    ...
    col=LINKCOL[kind]; d=DASH.get(kind,""); da=f' stroke-dasharray="{d}"' if d else ""
    sw="2.6" if kind=="interas" else "2.2"
...
# zones
A(f'<rect x="46" y="170" width="1160" height="800" rx="16" class="z-as1"/>')
A(f'<text x="66" y="200" class="zbig" fill="var(--sr1)">AS 65001 — your estate · OSPF-SR + EVPN/VXLAN</text>')
A(f'<rect x="78" y="222" width="1100" height="470" rx="12" class="z-dc"/>')
A(f'<text x="96" y="244" class="zsm" fill="var(--dc)">DC — EVPN/VXLAN · OSPF-SR underlay · tenants · dedicated market-data L3</text>')
A(f'<rect x="78" y="722" width="1100" height="230" rx="12" class="z-core"/>')
A(f'<text x="96" y="744" class="zsm" fill="var(--sr1)">Core — OSPF-SR · TI-LFA · Flex-Algo · ASBR1=core1</text>')
# inter-AS boundary
A(f'<line x1="46" y1="1002" x2="1206" y2="1002" stroke="var(--bd)" stroke-width="2.4" stroke-dasharray="11 7"/>')
A(f'<rect x="640" y="987" width="440" height="30" rx="15" fill="var(--card)" stroke="var(--bd)" stroke-width="1.3"/>')
A(f'<text x="860" y="1007" text-anchor="middle" class="bdlab">AS 65001 ⟷ AS 65002 · inter-AS SR-MPLS (BGP-LU)</text>')
# AS2 zone
A(f'<rect x="46" y="1032" width="1160" height="250" rx="16" class="z-as2"/>')
A(f'<text x="66" y="1062" class="zbig" fill="var(--sr2)">AS 65002 — your SR exit AS · OSPF-SR</text>')
...
for name,(cx,cy,role,lines) in NODES.items():
    ...
    if h>=62:
        ...
        A(f'<text x="{cx:.0f}" y="{cy+22:.0f}" text-anchor="middle" class="nlo">{lines[2]}</text></g>')
...
<title>TARGET architecture v2 — HRT lab (for approval)</title>
<style>
...
.nlo{{font:600 11px ui-monospace,monospace;fill:var(--ink)}}.llab{{font:700 11px sans-serif}}
...
<div class="wrap">
<h1>TARGET architecture — for approval (v2)</h1>
<p class="sub">2-AS SR-MPLS: EVPN/VXLAN DC (bl1 = border-leaf → core) · OSPF-SR core · <b>inter-AS SR via BGP-LU</b> → your AS2 exit · <b>EPE</b> peer vs transit · <b>dedicated PIM-SSM market-data</b> (own L3, not overlay) · <b>Traffic Dictator</b> driving SR-TE across <b>both</b> ASes. Not built yet.</p>
<div class="card">
...
<div class="legend">
 ...
 <span><i class="sw" style="background:var(--sr1)"></i>AS1 SR core / border-leaf</span>
 ...
 <span><i class="sw" style="background:var(--mcast)"></i>PIM-SSM market data</span>
 <span><i class="sw" style="background:var(--ctrl)"></i>controller</span>
</div>
<p class="note">
<b>bl1 is the border-leaf</b> — it uplinks to both spines like any leaf and it (not the spines) connects down to the core. <b>Market data is a dedicated L3 PIM-SSM segment</b> (<code>md-rx</code>, 10.9.0.0/24) off bl1, kept out of the VXLAN overlay: the (S,G) tree runs <code>mktdata</code> → edge2 → asbr2 → core → bl1 → md-rx (inter-AS SSM, IGMPv3 joins). h1/h2 are plain EVPN tenants. <b>Traffic Dictator peers both ASes</b> (BGP-LS + SR-Policy to core1 and asbr2) so it can steer inter-AS SR-TE and the EPE exit.
<br><b>15 nodes:</b> 10× cEOS (SR/EVPN) + 5× FRR/Linux (edges, hosts, feed). Loopbacks/ASNs = planned scheme.
</p>
</div>
"""
